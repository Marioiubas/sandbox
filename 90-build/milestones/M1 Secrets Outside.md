...
- 2026-09-25: automated: CI job `real-github` (Ubuntu 24.04, pushes to `main` only; skips without the `BROKER_GH_APP_KEY` secret; serialised) runs `code/tests/e2e/github_app_real/run.sh`: fast-forward push to `agent/ci` with a minted token, then `main`, force, delete and other-repository pushes refused with their reasons and checked with `broker why`, and GitHub checked from outside the broker. Passed locally against the real repositories before enabling.
