...
- [ ] `crates/policy/src/exporters/claude.rs` and `codex.rs`: emit Claude Code managed settings and Codex `requirements.toml` from the same Cedar source, as defence in depth ([[Native Config Exporters]]).
