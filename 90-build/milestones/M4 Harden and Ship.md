...
- 2026-09-25: `path_differential` fuzz target (generated inputs for the category 8 differential) found a third canonicaliser bypass (malformed escape stopping the decode); fixed; added to smoke and nightly fuzzing.
- 2026-09-25: E3 latency thresholds met on Linux and macOS (release builds, fair baseline): warm terminated added p50 3.1 ms / p95 3.1 ms; new terminated added p50 7.1 ms (8 clients) and 3.1 ms (1 client); warm spliced 0.0 / 0.7 ms; new spliced 3.8 / 2.0 ms; `broker run -- /usr/bin/true` p50 44.9 ms. Remaining for E3: added wall-clock per task (needs L3 runs). Follow-ups: upstreams with Nagle on (first request +~40 ms on Linux), the session trust bundle's load cost for OpenSSL 3 clients.
