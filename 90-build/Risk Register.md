...
| R16 | macOS per-user temp dir writable from the sandbox | Apple toolchain shims reset `TMPDIR` to it (measured, [[ADR-016 macOS M0 Compatibility Exceptions]]) | Medium | Medium | Broker state kept out by an overlap check; revisit with a CoW or per-session redirect | A sandboxed session modifies another program's temp file; a design partner asks for stricter temp isolation |
| R17 | Agents read their own credentials from the login keychain on macOS until M1 | Claude Code keeps its OAuth token there ([[ADR-016 macOS M0 Compatibility Exceptions]]) | High | Medium | Profile-scoped, announced and audited exception; M1 sentinel swap removes it | Closed 2026-09-24 by [[ADR-020 Brokered Agent Credentials End the Keychain Exception]]: the switch no longer exists |
| R18 | A brokered agent token expires mid-session | The Claude Code OAuth access token is read once per session from the keychain ([[ADR-020 Brokered Agent Credentials End the Keychain Exception]]) | Medium | Low | Re-read on 401 or add a broker-side OAuth refresh issuer; today the user re-runs the agent outside once | Users report 401s in long sessions |
| R19 | Legitimate pushes denied as force | Fast-forward is proven only inside the pushed pack ([[ADR-021 L7 Path Choices for M1]]) | Medium | Low | Explain in `broker why`; add an upstream ancestry check through the forge API with the broker's own credential | Design partners hit `not_provably_fast_forward` on normal workflows |
| R20 | Response-filter hold-back delays streamed tokens | The scanner holds back up to (longest needle - 1) bytes per chunk ([[ADR-021 L7 Path Choices for M1]]) | Low | Low | Measure p95 streaming latency in [[L4 Product Metrics]]; shorten needles to the secret alone when latency matters | Streaming latency regressions in the eval harness |
...
- 2026-09-25: **R10/L4 friction, new evidence.** Release builds meet every latency threshold on macOS; on the Linux CI runner warm requests and start time meet them; new connections first appeared to add 40-80 ms, which per-stage timings traced to the client loading the session trust bundle and to an upstream leaving Nagle on; with a fair baseline Linux meets every threshold too ([[L4 Product Metrics]]).
