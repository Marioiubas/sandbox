...
- 2026-09-25: mitigation for Nagle-on upstreams: on Linux the broker sets `TCP_QUICKACK` on its upstream socket after connect and after the TLS handshake (`brokerd::upstream::quickack`), so a server's reply is not held behind its session tickets until the broker's delayed ACK. The harness adds a Nagle-on upstream (`HttpsServer::start_with(.., false)`) to measure it on Linux.
- 2026-09-25: `TCP_QUICKACK` did not help: against a Nagle-on upstream the first terminated request is still answered at 42 ms inside the broker on Linux (CI run 36117466088), against 3.4 ms for a Nagle-off upstream. The change was reverted. **Known residual:** on Linux, an upstream that leaves Nagle on adds ~40 ms to the first request on each new terminated connection (later requests on the connection are unaffected); the exact interaction is not identified (a packet capture on Linux is the next step). The harness keeps the Nagle-on upstream so this stays measured.
- 2026-09-25: **cause identified** (loopback capture on the CI runner, run 36119834618): the broker's TLS client sends its handshake finish (80 B) and the request (121 B) in separate segments; the Nagle-on server sends its session tickets (184 B) at once and holds its 99 B response until they are acknowledged; the broker's kernel delays that ACK by exactly 40 ms because the socket has just sent data. The one-shot quick-ACK tried earlier was already undone by the broker's own writes. Fix: `brokerd::upstream::QuickAck` re-arms `TCP_QUICKACK` after every write on upstream TLS connections (Linux); re-measuring with the same capture.
- 2026-09-25: **fixed and confirmed** (CI run 36122330544): with `TCP_QUICKACK` re-armed after every upstream write, the capture shows the tickets acknowledged 12 µs after they arrive and the response following at once; warm terminated added p50 3.1 ms; new terminated 6.7 ms (8 clients) / 3.2 ms (1 client); new spliced 3.6 / 1.9 ms; first request to a Nagle-on upstream answered in 1.4 ms inside the broker (was 42 ms). The Nagle-on residual is closed; the harness keeps the Nagle-on upstream as a regression check. Remaining open: the session trust bundle's load cost for OpenSSL 3 clients.
- 2026-09-25: the trust-bundle follow-up is closed: it is not an added cost. Without the broker, curl and other OpenSSL clients on Ubuntu load the system bundle (`/etc/ssl/certs/ca-certificates.crt`, ~140 certificates) the same way; the session bundle (Mozilla roots plus the session CA and any extra roots, ~125) costs about the same. The one-CA baseline used first was the artificial case; the same-size baseline now in the harness is the fair one.
