...
**Implementation note (2026-09-25):** the stricter Willison variant compiles as two forbids on the GitHub write verbs and `git.push` (on `Repo`, where unknown visibility counts as public) and on `gist.create` and `pkg.publish` (on `Host`), behind an opt-in `[trifecta]` switch and with the Rule of Two's approval escape ([[ADR-033 Public-Sink Rule as Built]]).
...
- 2026-09-26 (M3): **implementation note** on the `rule-of-two` example above: as built (`code/policies/default.cedar`) it skips HTTP actions that a protocol adapter classified (`context.adapted`), and a second forbid, `rule-of-two-verbs`, applies the same condition to the GitHub and S3 write verbs, so a GraphQL read sent as a POST is not treated as a write ([[ADR-035 GitHub GraphQL and Host-Wide Read Labels as Built]]).
