...
- 2026-09-25 (M3): **fixed** a gap: host-wide GitHub reads (`github.read`: code search, `/user/repos`, `/orgs/{o}/repos`, notifications) did not raise `sensitive_read` although they can return private repositories, so a toxic flow through search passed the Rule-of-Two forbid. They now raise it unless the route is known to return public data only; GraphQL reads that leave one repository count the same way ([[ADR-035 GitHub GraphQL and Host-Wide Read Labels as Built]]). The HTTP method of a GitHub or S3 request no longer raises `external_effect` by itself (the verb decides). Tests: `m3_graphql::a_host_wide_search_is_a_sensitive_read`, `the_toxic_flow_over_graphql_is_stopped_at_the_public_write`.
- 2026-09-26 (M3): the Rule of Two on GitHub and S3 hosts now keys on the verb, not the HTTP method: GraphQL reads (POSTs) were refused once both labels were live. `rule-of-two` skips HTTP actions of adapted requests; `rule-of-two-verbs` covers `pr.create`, `pr.merge`, `issue.comment`, `contents.write`, `gist.create`, `s3.put`, `s3.delete` ([[ADR-035 GitHub GraphQL and Host-Wide Read Labels as Built]], item 11).
- 2026-09-26 (M3): **gap closed:** a git clone or fetch of a repository not known to be public now raises `sensitive_read` (anonymous `info/refs` probe per repository per session), so the toxic flow cannot take its private read through git ([[ADR-036 Git Fetch Visibility by Anonymous Probe]]).
- 2026-09-26 (M3): approvals built ([[ADR-037 Step-Up Approvals as Built]]): a write held by the Rule of Two (or `needs_approval`, or the public-sink rule) can be approved on the host for that exact action, once or for the session, after reviewing the authority diff and which requests raised the labels.
- 2026-09-26 (M3): **found by review:** with both labels live the agent's own model API calls (`POST` on a terminated grant) were refused by the Rule of Two, so the agent lost its model; grants marked `model_api = true` (the shipped profiles' model APIs) are exempt ([[ADR-039 Model API Is Not a Rule-of-Two Sink]]).
- 2026-09-26 (M3): **seven gaps found by a review agent and closed** ([[ADR-040 Label Sources Beyond the Adapters]]): maintainer-only GitHub routes, attacker text outside issues and pulls, credentialed reads on hosts without an adapter, intranet hosts (and the new grant key `sensitive`), push advertisements, the exact-path git probe, and pull-request refs over git. Tests: `m3_label_gaps_github::*`, `m3_label_gaps_git::*`.
