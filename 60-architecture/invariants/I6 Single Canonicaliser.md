...
- 2026-09-26 (M4): **found by fuzzing:** the first nightly fuzz run (2026-09-25, target `canon`) found that host canonicalisation was not a fixed point: `\u{ad}xn--xn--xn-n-nmm` (a soft hyphen before an A-label) was accepted as `xn--xn--xn-n-nmm`, which the ASCII path rejects (`idna_round_trip`); the non-ASCII path's check for input `xn--` labels ran before UTS 46 mapping dropped the soft hyphen. Fixed: the mapped name must be one the ASCII path accepts unchanged (`code/crates/netguard/src/canon.rs`, `unicode_name`); regression test `canon::tests::idna`. The same host spelled two ways was accepted once and refused once, the inconsistency this invariant forbids; no decision could have been widened by it (the broker resolves and connects to the canonical form it produced), but a fixed point is the property the tests require.
