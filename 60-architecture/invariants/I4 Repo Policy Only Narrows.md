...
milestone: M2
code: ["code/crates/brokerd/src/repo_policy.rs", "code/crates/broker-cli/src/cmd/policy.rs", "code/crates/policy/src/cedar/mod.rs", "code/crates/policy/src/gates/compose.rs"]
---
...
- 2026-09-24: the formal gate is built: `broker policy check --repo` proves `implies(conjoin(P, R), P)` over every request environment with SymCC, where `conjoin` encodes the runtime conjunction as one policy set; `gates::tests::the_repository_layer_only_narrows` (a repo-wide `permit` is proved narrowing; the union composition is shown to widen) and `conjoin_agrees_with_the_runtime` ([[ADR-024 Formal Gates as Built]]).
