...
- 2026-09-25 (M3): AWS: the agent holds a sentinel access key ID and a non-secret placeholder secret key; the base credentials and the minted session never enter the sandbox, its output or the audit log (`m3_s3::d6_s3_reads_through_minted_credentials_and_out_of_prefix_writes_are_denied`) ([[ADR-032 AWS STS and S3 Adapter as Built]]).
