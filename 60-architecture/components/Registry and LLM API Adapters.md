...
milestone: M1
code: ["code/crates/policy/src/l7.rs", "code/profiles/claude-code.toml", "code/profiles/claude-code-apikey.toml", "code/profiles/codex.toml", "code/profiles/gemini-cli.toml", "code/crates/policy/src/l7/credential.rs", "code/crates/policy/src/l7/secret.rs", "code/crates/policy/src/l7/explain.rs"]
---
...
- 2026-09-25: `code/crates/policy/src/l7.rs` split to stay under 500 lines, no behaviour change: credential declarations and their compilation moved to `l7/credential.rs`, secret references to `l7/secret.rs`, the explainer (`L7Decision`, `explain`) to `l7/explain.rs`; all re-exported under their old paths. The explainer's doc comment now matches ADR-027 (a plain grant allows nothing).
