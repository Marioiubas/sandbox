...
- 2026-09-26 (M3): REST labels widened ([[ADR-040 Label Sources Beyond the Adapters]]): `PUBLIC_ON_PUBLIC` sub-routes take the repository's visibility, every other repository read is private data; comments, events, files at pull-request refs, gists, notifications and repositories-by-ID carry bodies. GraphQL result reads are labelled (ADR-035 item 12).
