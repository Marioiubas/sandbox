...
- 2026-09-24 (M2): new commands `broker learn`, `broker suggest`, `broker policy status|approve|check|export|shadow`, `broker audit export` ([[ADR-023 OCSF and OTLP Export as Built]], [[ADR-024 Formal Gates as Built]], [[ADR-025 Native Config Exporters as Built]], [[ADR-026 Shadow Mode as Built]]). The daemon splits session start (`session/assemble.rs`: profile, user, repository and shadow layers; `session/running.rs`: accept loop and teardown) and the L7 answers (`l7_pipeline/respond.rs`) to keep files under 500 lines.
- 2026-09-25 (M3): `broker mcp connect <name>` (in-sandbox stub over the session proxy), `broker mcp approve <name> [--sha]`, `broker mcp list`; the daemon starts pinned servers as their own sessions (`session/mcp_launch.rs`) and relays them (`mcp/`) ([[ADR-030 MCP Guard as Built]]).
- 2026-09-25 (M3): CI mode: `BROKER_IDENTITY_TOKEN` (set by the GitHub Action) becomes a redacted `StartParams.identity_token`, verified by `brokerd::identity` before policy assembly; unverifiable tokens refuse the launch; `brokerd::fetch` for broker-originated HTTPS reads ([[ADR-031 CI Identity as Built]]).
- 2026-09-25: the launch step moved from `crates/brokerd/src/session.rs` to `crates/brokerd/src/session/launch.rs` to keep files under 500 lines (no behaviour change).
- 2026-09-25 (M3): `broker login` (device flow; `--status`) and `broker logout`; ctl methods `login.start`, `login.wait`, `login.status`, `login.logout`; `brokerd::login` holds identity and refresh token in memory and keeps the daemon up while a login is held; `brokerd::fetch` gained form POSTs and pinned addresses ([[ADR-034 Device Login as Built]]).
- 2026-09-25 (M4): `broker doctor` adds the per-grant TLS mode (terminated with protocol and credential, spliced, passthrough), identity, MCP and session-label settings, and the protocol matrix; it now compiles the user policy with its configured issuers (`m4_doctor`).
- 2026-09-26 (M3): step-up approvals ([[ADR-037 Step-Up Approvals as Built]]): control methods `approval.list` and `approval.grant`; commands `broker approvals [--json]` and `broker approve <id> [--for-session] [--yes]`; the daemon keeps pending approvals per running session (`code/crates/brokerd/src/approvals.rs`). `grant.request` is not built.
