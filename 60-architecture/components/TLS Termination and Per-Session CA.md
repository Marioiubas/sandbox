...
- 2026-09-25 (M4): upstream TLS connections go through `brokerd::upstream::QuickAck`, which re-arms `TCP_QUICKACK` after every write on Linux; without it a Nagle-on upstream held the first response ~40 ms behind its session tickets (found with a loopback capture; [[L4 Product Metrics]]).
- 2026-09-26 (M3): step 2 also refuses method-override headers (`X-HTTP-Method-Override`, `X-HTTP-Method`, `X-Method-Override`; reason `method_override`): a server that honours them would run another method than the one decided.
