...
- 2026-09-26 (M3): fetching a repository not known to be public raises `sensitive_read`; the broker probes `info/refs?service=git-upload-pack` anonymously on the same host once per repository per session ([[ADR-036 Git Fetch Visibility by Anonymous Probe]]; `code/crates/brokerd/src/l7_pipeline/github.rs`). Tests: `m3_git_labels::*`.
- 2026-09-26 (M3): upload-pack bodies are read: `refs/pull/*` requests (v2) and v0 fetches mark `pull_refs`, which raises `untrusted_input` on repositories not known to be private; push advertisements are probed and labelled like fetches; the probe uses the exact request path ([[ADR-040 Label Sources Beyond the Adapters]]). Tests: `git::tests::pull_request_refs_are_recognised_in_upload_pack_requests`, `m3_label_gaps_git::*`.
