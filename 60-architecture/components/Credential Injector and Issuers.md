...
milestone: M1
code: ["code/crates/creds/src", "code/crates/brokerd/src/session_l7.rs"]
---
...
- 2026-09-24: the issuers' own HTTPS calls go through `brokerd::upstream::BrokerTransport` (broker resolver, verified TLS, metadata/link-local addresses refused); issuer configuration (`[issuers.github_app.<name>]`) is user/org scope only. Agents' own login tokens are brokered by built-in profiles ([[ADR-020 Brokered Agent Credentials End the Keychain Exception]]).

## Build log

- 2026-09-24: built for M1: `creds` crate with `Secret` (zeroized, no `Clone`, redacted `Debug`), `sentinel` (`brk_s_` + 24 random bytes, bound to the credential's hosts, constant-time compare, opt-in body swap), `foreign` (`inspect`/`strip` over credential headers and query parameters), `secrets` (`keychain:`, `env:`, `file:`, `a|b` alternatives, JSON field selection), `issuers::github_app` (RS256 app JWT, request always carries `repositories` and `permissions`, response scope checked, `same_instance` check), `session::SessionCreds` (cache by scope digest, refresh margin 120 s, ttl bound). `AllowedBinding` (constructed only by `policy::l7`) is the only way to call `SessionCreds::get`. `aws_sts` and `oauth_exchange` remain [[M3 CI Identity and MCP]].
- 2026-09-24: tests: `creds` unit and property tests (22), including `mints_once_per_scope_and_never_without_allow`, `narrower_scope_gets_its_own_token`, `responses_beyond_the_request_are_refused`, `jwt_is_rs256_and_verifies`; conformance `b1_i1_only_sentinels_inside_the_sandbox`, `b4_i7_planted_key_to_allowed_host_is_rejected`, `b5_sentinel_copied_off_host_is_useless`, `b6_llm_call_succeeds_with_injected_key`, `b7_injected_secret_is_never_reflected`, `cat2_unmatched_paths_and_methods_deny` (`tests/conformance/tests/m1_secrets.rs`) and `b2_push_to_agent_branch_succeeds_with_scoped_token`, `b3_main_force_delete_and_other_repo_are_denied_and_explained`, `cat1_push_to_attacker_repo_mints_nothing` (`tests/conformance/tests/m1_git.rs`); fuzz target `foreign`.
- 2026-09-25 (M3): `issuers::aws_sts` (AssumeRole with the grant's session policy, base credentials from secret references, session cached per scope digest and zeroized with the run) and `issuers::sigv4`; `Issued` carries an AWS session's access key ID and token beside its secret key; `attach` refuses SigV4 specs (the pipeline signs the whole request with `aws_sts::sign_s3`); `needles` include the session token; sentinels for `aws_sts` fill `AWS_ACCESS_KEY_ID` plus a non-secret `AWS_SECRET_ACCESS_KEY` placeholder; `Transport::post` sends caller-signed requests ([[ADR-032 AWS STS and S3 Adapter as Built]]).
