...
milestone: M0
code: ["code/crates/launcher/src/lib.rs", "code/crates/launcher/src/fs_compile.rs", "code/crates/launcher/src/shim.rs", "code/crates/launcher/src/backends/seatbelt/sbpl.rs", "code/crates/launcher/src/backends/linux/bwrap.rs", "code/crates/launcher/src/backends/linux/inner.rs", "code/crates/launcher/src/backends/linux/inner/bridge.rs"]
---
...
- 2026-09-24 (M1): the `macos_keychain` switch is removed from profiles, `SandboxSpec` and the SBPL generator ([[ADR-020 Brokered Agent Credentials End the Keychain Exception]]); `SandboxSpec.ca_bundle` carries the session trust bundle path.
- 2026-09-25: `backends/linux/inner.rs` split to stay under 500 lines, no behaviour change: the in-namespace loopback-to-socket bridge moved to `linux/inner/bridge.rs` (checked and linted for the Linux target locally; tested on the Ubuntu CI runners).
