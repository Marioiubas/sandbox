...
- 2026-09-25: group commit built (`code/crates/audit/src/store/group.rs`): appends arriving during a commit go into the next one together; each caller returns after the commit holding its event; a failed commit fails every event in it and leaves the chain head unchanged. Tests `store::tests::concurrent_appends_share_commits_and_keep_one_chain`, `store::tests::a_failed_commit_fails_its_callers_and_leaves_the_chain_intact`. Measured append latency before it: macOS p50 0.10 ms, Linux runner p50 0.39 ms.
- 2026-09-25 (M3): reasons `github_graphql_invalid` and `github_graphql_mutation_unknown`; GitHub verb actions from GraphQL carry `graphql` (root field) and `node` (mutation node ID); an unresolved node's deny row carries `node_unresolved` ([[ADR-035 GitHub GraphQL and Host-Wide Read Labels as Built]]).
