...
12. **Fixes from an adversarial review** (2026-09-26, each with a regression test in `l7::github::graphql::tests` and `m3_graphql`): (a) a mutation's result could read the written repository (every issue, PR and comment) with no read verb, so no label was raised: item 8's "a confined payload needs no extra verb" was wrong. A result that selects more than the written objects' identity (`id`, `number`, `url`, `state`, …; `RESULT_OBJECTS`/`IDENTITY` in `code/crates/l7/src/github/graphql_walk.rs`) now adds a `repo.read` of the same node's repository, resolved and labelled like any read (`gh pr create`'s `{ pullRequest { id url } }` stays write-only). (b) A chain of about 12,000 fragments recursed once per link and overflowed a 2 MiB worker stack, aborting the daemon; walks are now bounded (`MAX_WALK_DEPTH` = 128). (c) One query could name thousands of repositories, each costing a visibility lookup with the user's credential; at most 16 (`MAX_REPOS`) per request.
