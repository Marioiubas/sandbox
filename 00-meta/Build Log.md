...
### 2026-09-25: M4 started (work that needs no outside accounts)

- **Milestone:** [[M4 Harden and Ship]] (in progress; [[M3 CI Identity and MCP]] waits only on outside accounts).
...
- **Correction 2:** audit appends take ~0.4 ms on the Linux runner, so they are not the cause either; the next suspect is debug-build CPU cost under 8 concurrent clients on 4 vCPUs, and the `latency` job now measures release builds. **Built:** group commit in the audit recorder (design point 3), with concurrency and failure tests.
- **Latency, release builds:** macOS meets every L4 threshold (splice warm 0.04 / 0.08 ms; splice new 3.1 / 4.0 ms; terminated warm 0.58 / 1.2 ms; terminated new 4.3 / 6.3 ms; start p50 35.3 ms); on Linux warm terminated (2.6 ms p50) and start (51 ms) meet them, and new connections add a fixed ~52 ms on both paths. **Found:** the pipeline's upstream socket lacked `TCP_NODELAY`; fixed.
- **Found (category 8 differential):** `canon_path` accepted `/allowed/..;/secret` and `/allowed/%C0%AE%C0%AE/secret` under `paths = ["/allowed/**"]`, which Tomcat-style or lenient servers serve as `/secret`. Fixed: `canon_path` now checks each segment at up to three percent-decoding levels: no dot segment, also behind `;params` or an escape (Tomcat's `..;/`, `..%3B`, `..%253B`), no encoded separator (`%252F`), no look-alike of a dot or slash (full-width, division slash), and valid UTF-8 (no overlong `C0 AE`). Test `m4_differential` (36 targets; forwarded paths are exactly the authorized ones and stay under the rule after server normalisation).
- **Found (fuzz, `path_differential`):** a third path-rule bypass, `/allowed/;;,..%25%255C../;/L;` (a malformed escape stopped the canonicaliser's decoding, lenient servers decode past it into a `\\`); fixed; 4.4 M generated paths then stayed under the rule.
- **Latency found:** most of the Linux new-connection gap was curl loading the ~125-certificate session trust bundle (OpenSSL 3, ~13 ms per load), which the one-CA direct baseline did not pay; the harness now compares against a same-size bundle. The broker's own stages on Linux: spliced connection ready at ~6 ms, first terminated request answered at ~3 ms. The trust bundle's client-side cost is a recorded follow-up.
- **Latency, settled:** with a fair baseline the broker meets every L4 threshold on Linux too (warm terminated added p50 3.1 ms / p95 3.1 ms; new terminated added p50 7.1 ms (8 clients) and 3.1 ms (1 client); warm spliced 0.0 / 0.7 ms; new spliced 3.8 / 2.0 ms; `broker run -- /usr/bin/true` p50 44.9 ms). The `TCP_NODELAY` changes stay (harmless, and the upstream socket one matters for some servers); the audit group commit stays (its design).
...
### 2026-09-25: M3 D3 against the real GitHub MCP server on Linux: pass

- **Ran:** CI job `real-mcp` (run 36166697216, Ubuntu, github-mcp-server v1.12.2 Linux release, checksum-pinned; fine-grained token for the two test repositories from the `BROKER_GH_MCP_TOKEN` Actions secret): manifest refused until approved; issue #1 and the private README read through the broker; `merge_pull_request` refused (`mcp_tool_not_allowed`); every allowed connection went to api.github.com; the token appeared in no reply and no audit row. [[M3 CI Identity and MCP]] D3 now passes against the real server.
